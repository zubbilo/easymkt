...
class BackupDevices(MikrotikBase, BasicCommand):
    ...
    def main(self,action,devices,details):
        ...
        for mikrotik in mikrotiks:
            try:
                ...
                logging.info(" ===> Starting backup for %s (%s)"%(name,addr))
            ...
            """Check exist backup before start create new"""
            if self.check_file_exist(filename,dirname,type='.backup'):
                logging.info("File(%s) with backup alredy exist."%(filename))
                report['status'] = STATUS_EXIST
            elif not report['error']:
                logging.info("Init backup for %s (%s)"%(name,addr))
                """Create backup file on mikrotik"""
                try:
                	bkp = self.MikrotikBackup(ssh,addr,self.parameters,filename)
                except MikrotikConnectException as msg:
                	report['error'] = msg
                	logging.warning(msg)
                except MikrotikException as msg:
                    ...
                    logging.info("Create backup success for %s (%s)"%(name,addr))
                finally:
                	report['status'] = STATUS_FAIL if report['error'] else STATUS_OK
            else:
                report['status'] = STATUS_FAIL

            """Get backup file from mikrotik"""
            ...
            if report['status'] == STATUS_FAIL: 
                ...
                if report['status'] == STATUS_OK:
                    """Clean backup file on mikrotik"""
                    try:
                	filename = report['file'] + '.backup'
                        logging.info("Removing Backup file %s from %s (%s)"%(filename,name,addr))
